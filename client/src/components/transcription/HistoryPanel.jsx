...
export const HistoryPanel = ({ onOpenAuth, onSelectTranscript, activeTranscriptId, refreshTrigger }) => {
  const { user } = useAuth();
  ...
  if (!user) {
    return (
      <Card className="flex flex-col items-center justify-center text-center p-8 min-h-[300px] border border-brand-border/55 bg-brand-card/60 shadow-[0_16px_30px_rgba(0,0,0,0.08)] select-none">
  ...
  return (
    <Card className="flex flex-col h-full min-h-[320px] p-5 relative border border-brand-border/55 bg-brand-card/60 shadow-[0_16px_30px_rgba(0,0,0,0.08)] select-none">
      {/* Header Toolbar */}
              ...
              {transcripts.map((item, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`p-3 border rounded-xl transition-all duration-250 text-left cursor-pointer group relative flex flex-col gap-1.5 ${
                      isActive
                        ? 'border-brand-accent/45 bg-brand-accent-light/20 shadow-[0_8px_18px_rgba(42,78,209,0.14)]'
                        : 'border-brand-border/60 hover:border-brand-accent/30 hover:bg-white/40 dark:hover:bg-slate-900/40'
                    }`}
                  >
      ...
      <AnimatePresence>
        {selectedTranscript && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-950/20 dark:bg-black/45 backdrop-blur-xs">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
